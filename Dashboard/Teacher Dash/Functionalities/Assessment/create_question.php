...
// Get current question count for this assessment
$current_question_count = $assessment->getCurrentQuestionCount($assessment_id);

// Add this near the top of the file where other assessment details are fetched
$remaining_questions = $assessment_details['question_count'] - $current_question_count;
$current_total_points = 0;
$existing_questions = $assessment->getQuestions($assessment_id);
foreach ($existing_questions as $q) {
    $current_total_points += $q['points'];
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_question') {
    try {
        // Check if question limit has been reached
        if ($current_question_count >= $assessment_details['question_count']) {
            throw new Exception("Cannot add more questions. Maximum limit of {$assessment_details['question_count']} questions reached.");
        }
        ...
        // Calculate current total points for this assessment
        $current_total = 0;
        $existing_questions = $assessment->getQuestions($assessment_id);
        foreach ($existing_questions as $q) {
            $current_total += $q['points'];
        }

        // Add the new question's points
        $new_total = $current_total + $_POST['points'];
        ...
        // Update the total points in the assessment table
        $assessment->updateTotalPoints($assessment_id, $new_total);
        ...
        <main class="main-content">
            ...
            <section id="assessment-details" class="section">
                ...
                <div class="assessment-info">
                    <p><Strong>Questions:</Strong> <?php echo $current_question_count; ?> / <?php echo $assessment_details['question_count']; ?></p>
                    <p><strong>Current Total Points: </strong> <?php echo number_format($current_total_points, 2); ?></p>
                    <p><strong>Remaining Questions:</strong> <?php echo $remaining_questions; ?></p>
                </div>
                ...
                <?php if ($current_question_count >= $assessment_details['question_count']): ?>
                    <div class="error-message">
                        Maximum number of questions (<?php echo $assessment_details['question_count']; ?>) has been reached.
                    </div>
                <?php endif; ?>

                <div class="form-section">
                    <h2>Add Question</h2>
                    <form action="" method="POST" class="question-form" <?php echo ($current_question_count >= $assessment_details['question_count']) ? 'style="display:none;"' : ''; ?>>
                        ...
                        <div class="form-group" id="options-section" style="display:none;">
                            ...
                            <button type="button" id="add-option">Add Option</button> <br> <br>
