...
class Mailer
{
    ...
    public function sendResetCode($email, $resetCode)
    {
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'emailsend675@gmail.com';  // account ko
            $mail->Password = 'daqd bfng lghd kpmj';  // password created din sa acct ko
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // STARTTLS encryption
            $mail->Port = 587;  // SMTP port for TLS

            // Email details
            $mail->setFrom('emailsend675@gmail.com', 'LMS-Reset Code');  // Sender email 
            $mail->addAddress($email);  // Recipient email/email from user

            $mail->isHTML(true);  // Send as HTML
            $mail->Subject = 'Reset Password';  // Subject of the email
            $mail->Body = "This is your code for password reset: <strong>$resetCode</strong>.<br>
